...
if (isset($_SESSION['user_session_id'])) {
	...
	$info 			= $route->getDetails(CART_TBL,"total_amount,coupon_value","id='$user_cart_id'");
	$amount_in_ps 	= ($info['total_amount']-$info['coupon_value'])*100;
	...
	if ($payment) {
		...
		header("location: ".BASEPATH."cart/details",  true,  301 );  
	    exit;
	}else{
		//echo"<script>window.location='".BASEPATH."'</script>";
		header("location: ".BASEPATH."cart/details",  true,  301 );  
