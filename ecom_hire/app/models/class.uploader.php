...
class Uploader {
    ...
    public $error_messages = array(
        ...
        'max_file_size' => "The Uploaded Image Size has exceeded 5 MB.",
        'max_number_of_files' => "Maximum number of files exceeded. Please Upload 10 Files at a time.",
	...
	private function prepareFiles(){
        ...
        if($validate){
            ...
            for($i = 0; $i < count($field['name']); $i++){
                ...
                $metas['img_name'] = $metas['name'];
                $metas['replaced'] = file_exists($metas['file']);
                $metas['date'] = date('r');
                
                if(!in_array($field['name'][$i], $removed_files) && $this->validate(array_merge($metas, array('index'=>$i, 'tmp'=>$tmp_name))) && $this->uploadFile($tmp_name, $metas['file'],$metas['img_name'])){
                    if($this->options['perms']) @chmod($metas['file'], $this->options['perms']);

                    $img = copy($metas['file'], $this->options['uploadDir'].$metas['img_name']);
                    $thump = $this->uploadThumpFile($this->options['uploadDir'],$this->options['uploadDir'].'thumbnail/',$metas['img_name']);
     ...
     * @return boolean
     */
    private function uploadFile($source, $destination,$img_name){
    ...
    private function uploadThumpFile($target_path,$thumb_path,$fileName)
    {   
        $filename_err       = explode(".",$fileName);
        ...
        imagecopyresized($thumb_create,$source,0,0,0,0,100,100,$width,$height);
        switch($file_ext){
            case 'jpg' || 'jpeg':
