	...
	function uploadFile($file_data,$title_name)
	{
		...
		$uploader  = new Uploader();
			...
			$data  = $uploader->upload($file_data['images'], array(
		        ...
		        'uploadDir'   => './resource/uploads/hire_uploads/', //Upload directory {String}
		...
		if (!empty($file_data['images']['name'][0])) {
		    ...
		    foreach ($data['data']['metas'] as $key => $value) {
		    	...
		    	// Rename File
		    	rename($value['file'], './resource/uploads/hire_uploads/'.$file_name);
		    	// Rename thumbnail name
		    	rename('./resource/uploads/hire_uploads/thumbnail/'.$img_name, './resource/uploads/hire_uploads/thumbnail/'.$thumb_file_name);
