...
class Uploader {
        ...
        if($validate){
            ...
            for($i = 0; $i < count($field['name']); $i++){
                ...
                if(!in_array($field['name'][$i], $removed_files) && $this->validate(array_merge($metas, array('index'=>$i, 'tmp'=>$tmp_name))) && $this->uploadFile($tmp_name, $metas['file'],$metas['img_name'])){
                    ...
                    //$img = copy($metas['file'], 'uploads/thumps/'.$metas['img_name']);
                    $thump = $this->uploadThumpFile('uploads/srcimg/','uploads/thumbnail/',$metas['img_name']);
    ...
    {   
        ...
        $thumb_create = imagecreatetruecolor(300,300);
        switch($file_ext){
            case 'jpg':
