...
class FileUploader extends Model
{
	...
	function uploadFile($file_data,$data)
	{
		$output = array();
		...
		if(!empty($file_data["file"]["type"])){
			...
			if ($validate==1) {
				...
				if (file_exists("./resource/uploads/".$temp_file_name)) {
				...
				$targetPath = "./resource/uploads/".$file_name;
	...
	// Add/Update Company Logo

	function uploadCompanyLogo($file_data,$data)
	{	
		// Output variables
		...
		if(count($file_data) > 0 ) {
			...
			foreach ($file_data as $key => $file) {
					$logo_name       = (($key=="header_logo")? "header logo" : "footer logo" );
					$validate        = $this->validateLogo($file,$logo_name);
					if($validate!=1) {
			...
			if(count($output['error']) == 0 ) {
				...
				foreach ($file_data as $key => $file) {
					...
					$temp_file_name = (($image_name=='') ? $rands.'_'.$date.'_'.$this->hyphenize($file['name']) : $image_name.".".$file_extension  );
					...
					move_uploaded_file($sourcePath,$targetPath);
					// Send output
					$output['file_name'][$key] 	= $file_name;
					
				}

				$output['is_uploaded'] = "ok";

			} else {

				$output['is_uploaded'] = "error";

			}

		} else {
			
			$output['is_uploaded']  = "empty";
	...
	function validateLogo($file,$logo_name)
	{
		$temporary      = explode(".", $file["name"]);
		$file_extension = end($temporary);
		$extensions     = array("jpeg", "jpg", "png", "JPG", "JPEG", "PNG");
		...
		if (in_array($file_extension, $extensions) && in_array($file["type"], $mime_types) ) {
			return 1;
		}else{
			return "Please upload ".$logo_name." in JPEG, JPG and PNG formats only. ";
		}
