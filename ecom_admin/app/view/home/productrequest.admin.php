<?php require_once 'includes/top.php'; ?> 
    ...
    <div class="modal fade zoom" tabindex="-1" id="ListDataModel">
            <form id="productRequestStatus" method="POST">
...
<script type="text/javascript">
    ...
    $('body').on('click','.close_prd_req_sts_modal',function() {
    ...
    $("#productRequestStatus").validate({
        submitHandler: function(form) {
            var formname = document.getElementById("productRequestStatus");
            var formData = new FormData(formname);
            $.ajax({
                url: core_path + "products/api/productRequestStatus",
                type: "POST",
                data: formData,
                contentType: false,
                cache: false,
                ...
                success: function(data) {
                    $(".page_loading").hide();
                    if (data == 1) {
                        window.location = core_path + "products/request?su=success";
                    } else {
                        $(".form-error").show();
...
setTimeout(function() {
    toastr.clear();
    NioApp.Toast('<h5>Product request status updated successfully !!</h5>', 'success', {
        position: 'bottom-center', 
        ui: 'is-light',
        "progressBar": true,
        "showDuration": "300",
