                            ...
                            <div class="col-md-10">
                                <div class="nk-block-head nk-block-head-sm">
                                    <div class="nk-block-between">
                                        ...
                                        <div class="nk-block-head-content">
                                            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#addStateModal"><em class="icon ni ni-plus"></em> Add State</button>
                                        </div>
                                    </div>
                                </div>
...
<!-- Add State -->
    <div class="modal fade zoom" tabindex="-1" id="addStateModal">
                        ...
                        <form id="addState" method="POST">
                            ...
                            <div class="form-group">
                                <p class="float-right model_pt">
                                    <button type="button" class="btn btn-light close_modal" data-modal_id="addStateModal">Cancel</button>
...
<!-- Edit State -->
    <div class="modal fade zoom" tabindex="-1" id="editStateModal">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <a href="#" class="close" data-dismiss="modal" aria-label="Close">
                    <em class="icon ni ni-cross"></em>
                ...
                <div class="modal-body">
                    <form id="editState" method="POST">
                        <?php echo $data['csrf_edit_state'] ?>
                        <div class="edit_state_modal">
...
<script type="text/javascript">
    ...
    $("#addState").validate({
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                type: "POST",
                url: core_path + "state/api/addState",
                dataType: "html",
                data: content,
                beforeSend: function() {
                    $(".page_loading").show();
                ...
                success: function(data) {
                    $(".page_loading").hide();
                    if (data == 1) {
                        window.location = core_path + "state?a=success";
                    } else {
                        $(".form-error").show();
    ...
    $(".changeStateStatus").click(function() {
        toastr.clear();
        var value = $(this).data("option");
        // alert(value);
        $.ajax({
            type: "POST",
            url: core_path + "state/api/statusState",
            dataType: "html",
            data: { result: value },
            beforeSend: function() {
                $(".page_loading").show();
    ...
    // Open Edit State Pop Up

    $(".openEditState").click(function() {
        toastr.clear();
        var value = $(this).data("option");
        $.ajax({
            type: "POST",
            url: core_path + "state/api/infoState",
            dataType: "html",
            data: { result: value },
            success: function(data) {
    ...
    $("#editState").validate({
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                type: "POST",
                url: core_path + "state/api/updateState",
                dataType: "html",
                data: content,
                beforeSend: function() {
                    $(".page_loading").show();
                ...
                success: function(data) {
                    $(".page_loading").hide();
                    if (data == 1) {
                        window.location = core_path + "state?e=success";
                    } else {
                        $(".form-error").show();
                        $(".form-error").html(data);
                        toastr.clear();
    ...
    $(".deleteState").click(function(e) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: "POST",
                    url: core_path + "state/api/deleteState",
                    dataType: "html",
                    data: { result: value },
                    beforeSend: function() {
                        $(".page_loading").show();
                    ...
                    success: function(data) {
                        $(".page_loading").hide();
                        if (data == 1) {
                            window.location = core_path + "state?d=success";
                        } else {
                            toastr.clear();
                            NioApp.Toast('<h5>'+data+'</h5>', 'error', {
                                position: 'bottom-center', 
...
setTimeout(function() {
    toastr.clear();
    NioApp.Toast('<h5>State added successfully !!</h5>', 'success', {
        position: 'bottom-center', 
        ui: 'is-light',
        "progressBar": true,
        "showDuration": "300",
...
setTimeout(function() {
    toastr.clear();
    NioApp.Toast('<h5>State Updated successfully !!</h5>', 'success', {
        position: 'bottom-center', 
        ui: 'is-light',
        "progressBar": true,
        "showDuration": "300",
...
setTimeout(function() {
    toastr.clear();
    NioApp.Toast('<h5>State Deleted successfully !!</h5>', 'success', {
        position: 'bottom-center', 
        ui: 'is-light',
        "progressBar": true,
        "showDuration": "300",
