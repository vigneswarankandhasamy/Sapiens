...
class exportData extends Controller
{
	...
	public function vendorrejectedorders($from="",$to="")
	{	
		if($from=="overall") {
			$from = "";
			$to = "";
		...
		if(isset($_SESSION["ecom_admin_id"])){
			...
			// Create Header
				$sheet->setActiveSheetIndex(0)
			            ...
			            ->setCellValue('H1', 'Commission')
			            ->setCellValue('I1', 'Order Value')
			           	->setCellValue('J1', 'Order Response')
			           	->setCellValue('K1', 'Response Message');			
			...
			$users = $user->exportVendorRejectedOrderReport($from,$to);
	...
	public function orderlistreport($from="",$to="",$vendor="")
	{	
		if(isset($_SESSION["ecom_admin_id"])){
			$user = $this->model('ExportReports');
			...
			if($vendor!="")
			{
				$vendor_detail = $user->getDetails(VENDOR_TBL,"company","id='".$vendor."'");
			...
			$users = $user->exportCustomerOrderList($from,$to,$vendor);
	...
	public function rejectedorderlistreport($from="",$to="",$vendor="")
	{	
		if(isset($_SESSION["ecom_admin_id"])){
			$user = $this->model('ExportReports');
			...
			// Set document properties
			$sheet->getProperties()->setCreator($vendor_name." Rejected Order List")
			               ->setLastModifiedBy(COMPANY_NAME)
			               ->setTitle($vendor_name." Rejected Order List")
			               ->setSubject($vendor_name." Rejected Order List")
			               ->setDescription($vendor_name." Rejected Order List")
			               ->setKeywords($vendor_name." Rejected Order List")
			               ->setCategory("Excel Report");
			// Set active sheet index to the first sheet, so Excel opens this as the first sheet
			$sheet->setActiveSheetIndex(0);
			$activeSheet = $sheet->getActiveSheet();
			...
			$sheet->setActiveSheetIndex(0)
			            ...
			            ->setCellValue('H1', 'Order Value')
			            ->setCellValue('I1', 'Commission')
			           	->setCellValue('J1', 'Response Notes')
			...
			$users = $user->exportVendorRejectedOrderReport($from,$to,$vendor);
	...
	public function cancelledorderlistreport($from="",$to="",$vendor="")
	{	
		if(isset($_SESSION["ecom_admin_id"])){
			$user = $this->model('ExportReports');
			...
			if($from!="" && $from!=0 ) {
				$from = date("d-M-y ",strtotime($from));
				$to   = date("d-M-y ",strtotime($to));
				$title 		= $vendor_name." Returned Order List (".$from." to ".$to.") ";
			} else if($from=='' || $from==0 ) {
				$title 		= $vendor_name." Returned Order List (".date("d-m-y").") ";
			} 
			...
			// Set document properties
			$sheet->getProperties()->setCreator($vendor_name." Returned Order List")
			               ->setLastModifiedBy(COMPANY_NAME)
			               ->setTitle($vendor_name." Returned Order List")
			               ->setSubject($vendor_name." Returned Order List")
			               ->setDescription($vendor_name." Returned Order List")
			               ->setKeywords($vendor_name." Returned Order List")
			               ->setCategory("Excel Report");
			// Set active sheet index to the first sheet, so Excel opens this as the first sheet
			$sheet->setActiveSheetIndex(0);
			$activeSheet = $sheet->getActiveSheet();
			...
			$users = $user->exportCustomerReturnedOrderList($from,$to,$vendor);
