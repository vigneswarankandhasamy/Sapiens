            ...
            <div class="col-12">
                <div class="breadcrumb_content">
                    <ul>
                        ...
                        <li><a href="<?php echo BASEPATH ?>calculator/concrete" class="breadcrumb_active">Concrete</a></li>
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="card list_wrap">
                    <h3>Concrete Calculation</h3>
                    <div class="row">
                        <div class="col-md-7 col-xs-12 col-sm-12">
                            <form id="concreteCalculation" method="POST" action="#">
                                <div class="row mb-3">
                                    <div class="col">
                                    ...
                                    <div class="col">
                                        <label for="inputEmail3" class="col-form-label">Grade of Concrete</label>
                                        <select class="form-select" name="gradeOfConcrete" id="gradeOfConcrete">
                                ...
                                <div class="row mb-3">
                                    <div class="col">
                                        <label for="inputEmail3" class="col-form-label">Depth</label>
                                        <div class="row">
                                             <div class="col-lg-3">
                                                <div class="input-group">
                                                    <div class="input-group-area">
                                                        <input type="number" class="form-control" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==6) return false;" name="depthMeterOrFeet" min="0"  id="depthMeterOrFeet" placeholder="meter">
                                                    </div>
                                                    <div class="input-group-icon depthMeterOrFeet">meter</div>
                                                </div>
                                                <div id="depthMeterOrFeetError">
                                                </div>
                                            </div>
                                            <div class="col-lg-3">
                                                <div class="input-group">
                                                    <div class="input-group-area">
                                                        <input type="number" class="form-control" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==3) return false;" name="depthCmOrInch" min="0"  id="depthCmOrInch" placeholder="cm">
                                                    </div>
                                                    <div class="input-group-icon depthCmOrInch">cm</div>
                                                </div>
                                                <div id="depthCmOrInchError">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="row">
                                    ...
                                    <table id='tableData' class='tableData'></table><br>
                                        <div class="chart" style="width: 65%">
                                            <canvas class="d-block" id="calconcerte" ></canvas>
                                        </div>
...
<script type="text/javascript">
    ...
    $("#unit").change(function() { 
       ...
       if(unit_val=="feet_or_inch") {
        ...
        $('#depthMeterOrFeet').attr('placeholder', 'feet');
        $('#depthCmOrInch').attr('placeholder', 'inch');

        $('.lengthMeterOrFeet').html('feet');
        $('.lengthCmOrInch').html('inch');
        $('.widthMeterOrFeet').html('feet');
        $('.widthCmOrInch').html('inch');
        $('.depthMeterOrFeet').html('feet');
        $('.depthCmOrInch').html('inch');
        ...
        $('#depthMeterOrFeet').attr('placeholder', 'meter');
        $('#depthCmOrInch').attr('placeholder', 'cm');

        $('.lengthMeterOrFeet').html('meter');
        $('.lengthCmOrInch').html('cm');
        $('.widthMeterOrFeet').html('meter');
        $('.widthCmOrInch').html('cm');
        $('.depthMeterOrFeet').html('meter');
        $('.depthCmOrInch').html('cm');
       }
    ...
    $.validator.addMethod("depthCmCheck", function (value, elem) {
        if($("#unit").val()=="meter_or_cm" ){
            if($("#depthCmOrInch").val() <= 99 ) {
                return true;
            } else {
                return false;
            }
    ...
    $.validator.addMethod("depthInchCheck", function (value, elem) {
        if($("#unit").val()=="feet_or_inch" ){
            if($("#depthCmOrInch").val() <= 11 ) {
                return true;
            } else {
                return false;
            }
   ...
   $("#concreteCalculation").validate({
    ...
    messages: {
        ...
        depthMeterOrFeet: {
            required: "Depth cannot be empty",
            digits: "Enter only numeric values decimal values are not allowed for this calculation",
        },
        lengthCmOrInch: {
        ...
        depthCmOrInch: {
            depthCmCheck : "Depth between 0 to 99 cm",
            depthInchCheck : "Depth between 0 to 11 inch",
            digits: "Enter only numeric values decimal values are not allowed for this calculation",
        }
    ...
    errorPlacement : function (error, element) {
        ...
        } else if(element.attr("name") == "depthMeterOrFeet") {
            error.appendTo("#depthMeterOrFeetError");
        ...
        } else if(element.attr("name") == "depthCmOrInch") {
            error.appendTo("#depthCmOrInchError");
        }  
    },
    ...
    submitHandler: function (form) {
        ...
        $.ajax({
            type: "POST",
            url: base_path + "calculator/api/concreteCalculation",
            dataType: "html",
            data: content,
            beforeSend: function () {
                $(".page_loading").show();
            ...
            success: function (data) {
                ...
                brickworkCalculationChart(json["cement"],json["sand"],json["sand_cft"],json["aggregate"],json["aggregate_cft"]);
            }
        });
        return false;
    } 
    ...
    function brickworkCalculationChart(cement,sand,sand_cft,aggregate,aggregate_cft) {
        var options = {
            // legend: false,
            responsive: true
        };

        $('#tableData').html( '<tr><th>' + 'Cement' + '</th><td>' + cement + ' Bags' + '</td></tr><tr><th>' + 'Sand' + '</th><td>' + sand + ' Tons'+ ' (' + sand_cft + 'CFT)</td></tr><tr><th>' + 'Aggregate' + '</th><td>' + aggregate + ' Tons'+ ' (' + aggregate_cft + 'CFT)</td></tr>' );
        
        let init = function() {
            chart_obj = new Chart($("#calconcerte"), {
                type: 'doughnut',
                ...
                data: {
                    labels: [
                        ...
                        `Aggregate - ${aggregate} Tons (${aggregate_cft}` + ` CFT)`
                    ],
                    datasets: [{
                        data: [cement , sand, aggregate],
                        backgroundColor: [
                            "#3498DB",
                            "#ffc107",
                            "#9B59B6"
